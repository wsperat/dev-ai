...
Proxmox host
  └── Ubuntu Server VM, q35 + OVMF + NVIDIA PCIe passthrough
        ...
        └── Docker + NVIDIA Container Toolkit
              ...
              └── optional OpenHands UI container
```

The primary Claude Code replacement here is **OpenCode + Ollama** for terminal-based agentic coding. I’d also install **aider** for precise Git-based editing, **Goose** for general agent workflows/MCP-style extension work, and **OpenHands** as an optional browser UI for more autonomous repo work. OpenCode is an open-source terminal coding agent, aider connects to local Ollama models, Goose supports CLI/Desktop/API usage and Ollama among other providers, and OpenHands can be run locally with Docker/`uv` and optional GPU support. ([OpenCode][2])
...
## 12. Optional: run OpenHands UI

The Compose file includes OpenHands behind a `ui` profile. Start it:
...
In the OpenHands UI, configure a local model endpoint:

```text
Provider: local / custom / Ollama-compatible, depending on UI options
Base URL: http://host.docker.internal:11434
API key: local-placeholder
Model: devstral
```

OpenHands’ local setup docs show both a `uv` launcher path and a direct Docker path; the Docker example mounts `/var/run/docker.sock`, uses `docker.openhands.dev/openhands/openhands:1.7`, and exposes port `3000`. Treat this as privileged because mounting the Docker socket is effectively root-equivalent inside this VM. ([docs.openhands.dev][13])
...
docker image inspect --format='{{index .RepoDigests 0}}' docker.openhands.dev/openhands/openhands:1.7
```
...
### OpenHands can access too much

OpenHands mounts the Docker socket. Keep it bound to localhost, use this VM as a security boundary, and snapshot the VM before giving agents broad tasks.
...
[13]: https://docs.openhands.dev/openhands/usage/run-openhands/local-setup "Setup - OpenHands Docs"
